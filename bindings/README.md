...
catchable by an automated test).

Neither binding pulls in geometry — poncelet never owns collision (the `World`
callback in the C++ API, not yet in the C ABI). Hit detection against your
engine's physics is the integrator's job; feed poncelet the muzzle transform and
read back positions.
