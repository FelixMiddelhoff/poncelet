...
# src/integrate.cpp is built by CMake with /fp:precise /fp:except- (MSVC) or
# -ffp-contract=off -fno-fast-math (GCC/Clang). A single-TU build loses that
# per-file flag; re-assert what a pragma can. A whole-TU fast-math build still
# can't promise cross-platform BitExact from the single header — use the
# multi-file build for that (see dist/README.md).
